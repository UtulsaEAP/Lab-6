...
template_error = "Make sure to put a comma after each number without a space. IE 1,2,3,"
def test_first_input(capsys):
    input = [25, 51, 0, 200, 33]
    min_val, max_val = 0, 50
    filter_and_print_range(input, min_val, max_val)
    output = capsys.readouterr().out
    assert output == "25,0,33,", template_error
...
def test_second_input(capsys):
    ...
    filter_and_print_range(input, min_val, max_val)
    output = capsys.readouterr().out
    assert output == "100,75,", template_error
...
def test_three_input(capsys):
    ...
    filter_and_print_range(input, min_val, max_val)
    output = capsys.readouterr().out
    assert output == "1,2,3,", template_error
...
def test_four_input(capsys):
    ...
    filter_and_print_range(input, min_val, max_val)
    output = capsys.readouterr().out
    assert output == "1,2,3,4,5,", template_error
...
def test_six_input(capsys):
    ...
    filter_and_print_range(input, min_val, max_val)
    output = capsys.readouterr().out
    assert output == "1,", template_error
